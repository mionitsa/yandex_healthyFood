...
def handle_dialog(req, res):
    ...
    if 'совет' in req['request']['original_utterance'].lower():
        res['response']['text'] = 'Вот советы по правильному питанию:\n1. Следите за своим рационом, а также регулярностью питания.\n' \
    ...
    if  req['request']['original_utterance'].replace('.','') == '1':
        res['response']['text'] = 'Питание должно быть сбалансированным и включать ' \
        'белки, жиры, углеводы, витамины, макро- и микроэлементы. Оптимальный ежедневный рацион на 50% состоит ' \
        'из углеводов, на 25% — из белков и на 25% — из жиров. '
        res['response']['buttons'] = [
            {'title': "Что такое БЖУ?",  'hide': True}
        ]
        return

    if req['request']['original_utterance'].replace('.','') == '2':
    ...
    if req['request']['original_utterance'].replace('.','') == '3':
    ...
    if req['request']['original_utterance'].replace('.','') == '4':
    ...
    if req['request']['original_utterance'].replace('.','') == '5':
    ...
    if req['request']['original_utterance'].replace('.','') == '6':
        ...
        return
    #БЖУ
    if 'бжу' in req['request']['original_utterance'].lower():
        res['response']['text'] = 'БЖУ - это сокращённое название Белков, Жиров и ' \
    ...
    if 'углевод' in req['request']['original_utterance'].lower():
        res['response']['text'] = '• Углеводы - главный поставщик энергии, сюда входят также клетчатка, витамины и минералы. Они часто сконцентрированы в богатых углеводами продуктах. ' \
    ...
    if 'здоров' req['request']['original_utterance'].lower():
    ...
    if 'факт' in req['request']['original_utterance'].lower():
        ...
        'Через 21 минуту после еды исчезает чувство голода у человека.',
        'К смертельному отравлению могут привести яблочные косточки.',
        'В современном мире более 2400 км преодолевает пища перед тем, как попасть на стол.',]
    ...
    if 'клик' in str(req['request']['original_utterance']).lower():
        res['response']['text'] = 'А вы знали, что болле 5 лет своей жизни человек проводит за едой. Я знаю куча таких интересных фактов, спрашивай!'
        return

    if 'вод' in str(req['request']['original_utterance']).lower():
