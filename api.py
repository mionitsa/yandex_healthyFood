...
def handle_dialog(req, res):
    ...
    if 'факт' in req['request']['original_utterance'].lower():
        ...
        'Более 5 лет своей жизни в среднем человек проводит за едой.',
        'Через 21 минуту после еды исчезает чувств голода у человека.',
        'Примерно 5 лет человек тратит на приём пищи за жизнь.',
    ...
    if 'клик' in str(req['request']['original_utterance']).lower():
        res['response']['text'] = 'Интересный факт: %s Я знаю куча таких!' % (
        bank[random_number]
        )
        return
